...
describe('distributeYield', () => {
  it('gibt dem Besitzer einer Siedlung eine Karte je passendem Feld', () => {
    const state = testGame({
      buildings: {
        [HILLS_VERTEX_A]: { owner: 'p1', kind: 'settlement', wall: false, metropolis: null },
      },
    });
  ...
  it('gibt einer Stadt zwei Karten', () => {
    const state = testGame({
      buildings: { [HILLS_VERTEX_A]: { owner: 'p1', kind: 'city', wall: false, metropolis: null } },
    });
  ...
  it('bedient mehrere Spieler am selben Feld', () => {
    const state = testGame({
      buildings: {
        [HILLS_VERTEX_A]: { owner: 'p1', kind: 'settlement', wall: false, metropolis: null },
        [HILLS_VERTEX_B]: { owner: 'p2', kind: 'city', wall: false, metropolis: null },
      },
    });
  ...
  describe('wenn die Bank knapp wird', () => {
    it('gibt einem einzelnen Anspruchsberechtigten, was noch da ist', () => {
      const state = testGame({
        buildings: {
          [HILLS_VERTEX_A]: { owner: 'p1', kind: 'city', wall: false, metropolis: null },
        },
