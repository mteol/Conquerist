...
describe('BoardSvg', () => {
  ...
  /**
   * Die Untiefen liegen **unter** dem Brett - und das ist die ganze Mechanik.
   *
   * Sie sind dieselben neunzehn Sechsecke wie die Felder, nur als breite
   * Kontur. Was nach innen ragt, verschwindet unter den gefuellten Feldern;
   * sichtbar bleibt genau der Ueberstand zur See. Rutscht die Gruppe im Markup
   * **hinter** `.hexfields`, kehrt sich das um: dann liegt ein milchiger
   * Pergamentschleier ueber jedem Feld, jeder Strasse und jedem Haus, und die
   * Kueste ist verschwunden. Das ist kein Fehler, den ein Blick aufs Brett
   * sofort als Ursache benennt - im Zweifel sucht man ihn in der Deckkraft.
   *
   * Geprueft wird deshalb die Reihenfolge im Dokument und nicht das Aussehen.
   */
  it('legt die Untiefen unter die Felder und nicht darueber', () => {
    ...
    const coast = container.querySelector('.coast')!;
    const fields = container.querySelector('.hexfields')!;

    expect(coast, 'der Kuestensaum fehlt').not.toBeNull();
    expect(
      coast.compareDocumentPosition(fields) & Node.DOCUMENT_POSITION_FOLLOWING,
      'die Untiefen stehen hinter den Feldern - damit liegen sie darueber',
    ).toBeTruthy();
  });

  /**
   * Jeder Saum zeichnet **jedes** Feld nach, nicht nur die am Rand.
   *
   * Es waere verlockend, nur die Kuestenfelder zu saeumen und sich die
   * inneren zu sparen. Genau das ginge schief: welche Felder am Rand liegen,
   * haengt am Szenario, und ein Szenario mit einem Loch in der Mitte (oder
   * mit zwei Inseln) haette dort eine Kueste ohne Saum. Alle neunzehn zu
   * zeichnen kostet nichts und stimmt fuer jede Form, die das Brett annehmen
   * kann - die Ueberdeckung erledigt die Auswahl von selbst.
   */
  it('saeumt in jedem Band jedes Feld des Szenarios', () => {
    const { container } = render(
      <BoardSvg
        state={start}
        targets={actionTargets(start, 'p1')}
    ...
    const bands = container.querySelectorAll('.coast > g');

    expect(bands.length, 'es sind nicht drei Baender').toBe(3);

    let last = Infinity;
    for (const band of bands) {
      const polygons = band.querySelectorAll('polygon');

      expect(polygons.length, `${band.getAttribute('class')} saeumt nicht jedes Feld`).toBe(
        scenario.hexes.length,
      );

      /*
       * Von aussen nach innen immer schmaler - stuende ein breites Band hinter
       * einem schmalen, waere es das schmale, das man sieht, und die Stufung
       * kaeme genau verkehrt heraus.
       */
      const width = Number(polygons[0]!.getAttribute('stroke-width'));

      expect(
        width,
        `${band.getAttribute('class')} ist nicht schmaler als das Band davor`,
      ).toBeLessThan(last);
      last = width;
    }
  });
